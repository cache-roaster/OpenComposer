...
class Scheduler
  ...
  # @param bin_overrides [String] executable path of a job scheduler.
  ...
  def query(job_ids, bin_overrides = nil, ssh_wrapper = nil)
    raise NotImplementedError, "This method should be overridden by a subclass"
  end
  ...
  def get_command_path(command, bin_overrides)
    bin_overrides&.fetch(command, command) || command
  end
end
